...
module.exports = {
    ...
    testRubyFileParsePluralMissingRequiredKey: function(test) {
        ...
        rf.parse("Rb.p(:other => \"There are %{count} tests\", :variables => {count: 1})");
        var set = rf.getTranslationSet();
        test.ok(set);
        test.equal(set.size(), 0);
        test.done();
    ...
    testRubyFileParsePluralSkipInvalidKey: function(test) {
        ...
        rf.parse("Rb.p(one: 'This is 1 test', :three => \"There are %{count} tests\", :variables => {count: 1})");
        var set = rf.getTranslationSet();
        test.ok(set);
        test.equal(set.size(), 1);
        var r = set.getAll()[0];
    ...
    testRubyFileParsePluralAllValid: function(test) {
        ...
        rf.parse("Rb.p(one: 'This is 1 test', two: 'There are a couple tests', zero: 'There are no tests', few: 'There are a few tests', many: 'There are many tests', :other => \"There are %{count} tests\", :variables => {count: 1})");
        var set = rf.getTranslationSet();
        test.ok(set);
        test.equal(set.size(), 1);
        var r = set.getAll()[0];
