...
Xliff.prototype.addResource = function(res) {
	...
	var hashkey = [res.getPath(), res.getLocale(), res.getKey(), res.getContext()].join("_");
	
	if (this.reshash[hashkey]) {
		var oldres = this.reshash[hashkey];
		for (var p in res) {
			oldres[p] = res[p];
		}
	} else {
		this.resources.push(res);
		this.reshash[hashkey] = res;
	}
};
...
Xliff.prototype.getResources = function(criteria) {
	if (!criteria) return this.resources;
...
Xliff.prototype.deserialize = function(xml) {
	...
	logger.trace("json is " + JSON.stringify(json, undefined, 4));
	var ts = new TranslationSet(this.sourceLocale);
				...
				if (file.body && file.body["trans-unit"]) {
					...
					for (var j = 0; j < units.length; j++) {
						...
						case "array":
							...
							var res = ts.get(tu.resname, "array", tu["x-context"], fileSettings.locale, fileSettings.project);
							if (res) {
							...
							if (tu.target) {
								var res = ts.get(tu.resname, "plural", tu["x-context"], fileSettings.targetLocale, fileSettings.project);
						...
						case "plural":
							var strings = {};
							var res = ts.get(tu.resname, "plural", tu["x-context"], fileSettings.locale, fileSettings.project);
							if (res) {
								res.addString(tu.extype, tu.source["$t"]);
							} else {
								strings[tu.extype] = tu.source["$t"];

								ts.add(new ResourcePlural({
									pathName: fileSettings.pathName,
									locale: fileSettings.locale,
									project: fileSettings.project,
									id: tu.id,
