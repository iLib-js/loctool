...
Xliff.prototype.addTranslationUnit = function(unit) {
    ...
    if (oldUnit && !this.allowDups) {
        ...
        if (this.version && this.version.startsWith("2") && this.tu.length) {
...
Xliff.prototype.serialize = function(untranslated) {
    ...
    return (!this.version || this.version.startsWith("1")) ? this.toString1(units) : this.toString2(units);
};
...
Xliff.prototype.parse2 = function(xliff) {
    ...
    if (xliff.file) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (file.unit) {
                var units = makeArray(file.unit);

                units.forEach(function(tu) {
                    var comment, state;
                    var datatype = tu["l:datatype"];
                    var source = "", target = "";
...
/**
 * Factory method to create a new xliff instance from the file given in pathName. This
 * static method will read the xliff file and parse it properly for the value of the
 * xliff version attribute on the xliff tag.
 *
 * @throws Error "File not found" if the given pathName does not name a readable xliff file on disk
 * @param {String} pathName path to the xliff file to read
 * @returns {Xliff} an xliff instance that represents that file
 */
Xliff.createXliff = function(pathName) {
    if (!fs.existsSync(pathName)) {
        throw new Error("File not found");
    }

    var xml = fs.readFileSync(this.pathName, "utf-8");
    ...
    if (json && json.xliff) {
        if (json.xliff.version) {
            if (json.xliff.version === "1.2") {
                return new Xliff12({json: json});
            } else if (json.xliff.version === "2.0") {
                return new Xliff20({json: json});
            }
        }
    } else {
        throw new Error("Not an xliff file");
    }
