...
Xliff.prototype.getTranslationSet = function() {
	...
	if (this.tu) {
		for (var j = 0; j < this.tu.length; j++) {
			...
			case "array":
				var arr;
				var res = this.ts.get(tu.key, "array", tu.context, tu.sourceLocale, tu.project, tu.file);
				if (res) {
				...
				if (tu.target) {
					var res = this.ts.get(tu.key, "array", tu.context, tu.targetLocale, tu.project, tu.file);
			...
			case "plural":
				var strings = {};
				var res = this.ts.get(tu.key, "plural", tu.context, tu.sourceLocale, tu.project, tu.file);
				if (res) {
				...
				if (tu.target) {
					var res = this.ts.get(tu.key, "plural", tu.context, tu.targetLocale, tu.project, tu.file);
