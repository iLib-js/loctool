 ...
 * <li><i>target</i> - source text for this unit (optional)
 ...
 * @returns
 */
Xliff.prototype._hashKey = function(project, context, sourceLocale, targetLocale, key, type, path, ordinal, quantity) {
	var key = [key, type || "string", sourceLocale || this.sourceLocale, targetLocale || "", context || "", project, path || "", ordinal || "", quantity || ""].join("_");
...
Xliff.prototype.addTranslationUnit = function(unit) {
	...
	var hashKeySource = this._hashKey(unit.project, unit.context, unit.sourceLocale, "", unit.key, unit.resType, unit.file, unit.ordinal, unit.quantity),
		hashKeyTarget = this._hashKey(unit.project, unit.context, unit.sourceLocale, unit.targetLocale, unit.key, unit.resType, unit.file, unit.ordinal, unit.quantity);
...
Xliff.prototype.addResource = function(res) {
	...
	if (res.origin === "target" && (res.locale === this.sourceLocale || res.locale === "en")) {
		...
		for (var j = 0; j < this.tu.length; j++) {
			...
			default:
				this.ts.add(ResourceFactory({
					...
					sourceHash: utils.hashKey(tu.source.replace(/\n/g, '')),
					state: tu.state
				}));
				if (tu.target) {
					this.ts.add(ResourceFactory({
			...
			case "array":
				var arr;
				var res = this.ts.get(ResourceArray.hashKey(tu.project, tu.context, tu.sourceLocale, tu.key));
				if (res) {
				...
				if (tu.target) {
					var res = this.ts.get(ResourceArray.hashKey(tu.project, tu.context, tu.targetLocale, tu.key));
					
					if (res) {
						res.addString(tu.ordinal, tu.target);
					} else {
						arr = [];
						arr[tu.ordinal] = tu.target;
			...
			case "plural":
				var strings = {};
				var res = this.ts.get(ResourcePlural.hashKey(tu.project, tu.context, tu.sourceLocale, tu.key));
				if (res) {
				...
				if (tu.target) {
					var res = this.ts.get(ResourcePlural.hashKey(tu.project, tu.context, tu.targetLocale, tu.key));
			...
			if (res.origin === "source" || this.sourceLocale === res.getLocale()) {
				try {
					...
					case "string":
						...
						sourceUnitHash[res.hashKey()] = new TranslationUnit({
					...
					case "array":
						...
						for (var j = 0; j < arr.length; j++) {
							sourceUnitHash[res.hashKey() + "_" + j] = new TranslationUnit({
								source: arr[j],
								sourceLocale: res.getLocale(),
								key: res.getKey(),
								file: res.getPath(),
					...
					case "plural":
						if (res.reskey === "r1072022560") {
							logger.trace("Got it!");
						}
						var plurals = res.getPlurals();
						for (var p in plurals) {
							sourceUnitHash[res.hashKey() + "_" + p] = new TranslationUnit({
								source: plurals[p],
								sourceLocale: res.getLocale(),
								key: res.getKey(),
								file: res.getPath(),
		...
		for (var i = 0; i < resources.length; i++) {
			...
			if (res.origin === "target" || res.getLocale() !== this.sourceLocale) {
				try {
					switch (res.resType) {
					case "string":
						var tu = sourceUnitHash[res.hashKeyForTranslation(this.sourceLocale)];
						if (tu) {
							tu.translated = true;
							var newtu = tu.clone();
					...
					case "array":
						...
						for (var j = 0; j < arr.length; j++) {
							...
							if (arr[j]) {
								var tu = sourceUnitHash[res.hashKeyForTranslation(this.sourceLocale) + "_" + j];
								if (tu) {
									tu.translated = true;
									var newtu = tu.clone();
									newtu.target = arr[j];
					...
					case "plural":
						var plurals = res.getPlurals();
						for (var p in plurals) {
							var tu = sourceUnitHash[res.hashKeyForTranslation(this.sourceLocale) + "_" + p];
							if (tu) {
								tu.translated = true;
								var newtu = tu.clone();
