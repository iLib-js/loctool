	    	...
	    	if ((name === "title" || (localizableAttributes[lastTagName] && localizableAttributes[lastTagName][name])) &&
	    		...
	    		if (this.tagtext) {
		    		this.segments.enqueue({
		    			...
		    			text: escapeQuotes(source),
		    			replacement: '{' + name + '}'
		    		});

		    		this.tagtext += " " + name + '="{' + name + '}"';
	    		} else {
	    	    	this.accumulator += " " + name + '=' + value[0];

	    	    	this.text = escapeQuotes(source);
	    	    	this._emitText();

	    	    	this.accumulator += value[0]; // trailing quote same as opening quote
	    		}
    		} else {
    			// non-localizable and values containing template tags just get added without localization
    			if (this.tagtext) {
		    		this.tagtext += " " + name + '=' + escapeQuotes(value);
    			} else if (this.text) {
		    		this.text += " " + name + '=' + escapeQuotes(value);
	    		} else {
	    			this.accumulator += " " + name + '=' + escapeQuotes(value);
	    		}
    		}
	    }.bind(this),
		...
		if (segment.localizable) {
			if (segment.attributeSubstitution) {
				if (locale === this.project.pseudoLocale) {
					substitution = this.type.pseudoBundle.getStringJS(segment.text);
				} else if (this.type && this.type.pseudos[locale]) {
					substitution = this.type.pseudos[locale].getStringJS(segment.text);
				} else {
					var translated = translations.getBy({
						project: this.project.getProjectId(),
						locale: locale,
						reskey: this.makeKey(escapeInvalidChars(segment.text))
					});
				...
				if (locale === this.project.pseudoLocale && !this.project.settings.nopseudo) {
					additional = this.type.pseudoBundle.getStringJS(segment.text);
				} else if (this.type && this.type.pseudos[locale]) {
					additional = this.type.pseudos[locale].getStringJS(segment.text);
				} else {

					var translated = translations.getClean(
						ResourceString.cleanHashKey(
								this.project.getProjectId(), locale, this.makeKey(escapeInvalidChars(segment.text)), "html"));
					var additional;
					if (translated) {
						...
						if (this.type && containsActualText(segment.text)) {
							...
							additional = this.project.settings.nopseudo && locale === this.project.pseudoLocale ? segment.text : this.type.pseudoBundle.getStringJS(segment.text);
						} else {
