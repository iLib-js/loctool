...
HTMLTemplateFile.prototype.getLocalizedPath = function(locale) {
	...
	if (parts.length > 2) {
		fileName = parts.splice(parts.length-2, 0, locale).join(".");
	} else if (parts.length > 1) {
		fileName = parts.splice(parts.length-1, 0, locale).join(".");
	} else {
...
HTMLTemplateFile.prototype.localizeText = function(translations, locale) {
	...
	while (fragment) {
		if (fragment.localizable) {
			if (fragment.attributeSubstitution) {
				var translated = translations.getBy({
					project: this.project.getProjectId(),
					locale: locale,
					reskey: fragment.text
				});
				substitution = translated && translated.length ? translated[0].getSource() : fragment.text;
				...
				var additional = translated && translated.length ? translated[0].getSource() : fragment.text;
				if (substitution) {
