...
SwiftFileType.prototype.write = function(translations, locales) {
	...
	for (var i = 0; i < resources.length; i++) {
		res = resources[i];
		file = resFileType.getResourceFile(res.locale, res.pathName, "swift");
		...
		translationLocales.forEach(function(locale) {
			...
			db.getResourceByCleanHashKey(res.cleanHashKeyForTranslation(locale), function(err, translated) {
				...
				file = resFileType.getResourceFile(locale, res.pathName, "swift");
				file.addResource(r);
	...
	for (var i = 0; i < resources.length; i++) {
		res = resources[i];
		file = resFileType.getResourceFile(res.locale, undefined, "swift");
		file.addResource(res);
...
SwiftFileType.prototype.newFile = function(path) {
	return new SwiftFile(this.project, path);
};
...
SwiftFileType.prototype.registerDataTypes = function() {
	ResourceFactory.registerDataType("x-swift", "string", ResourceString);
};
