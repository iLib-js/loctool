...
//var reGetStringSingle = new RegExp("\\bRB\\.getString\\s*\\(\\s*'([^']*)'\\s*\\)", "g");
//var reGetStringSingleWithId = new RegExp("\\bRB\\.getString\\('([^']*)',\\s*'([^']*)'\\)", "g");
var reGetStringDouble = new RegExp(/(^R|\WR)B\.getString(JS)?\s*\(\s*("([^"]*)"|'([^']*)')\s*\)/g);
var reGetStringDoubleWithId = new RegExp(/(^R|\WR)B\.getString(JS)?\s*\(\s*("([^"]*)"|'([^']*)')\s*,\s*("([^"]*)"|'([^']*)')\s*\)/g);
...
JavaScriptFile.prototype.parse = function(data) {
	...
	reGetStringDouble.lastIndex = 0; // just to be safe
	...
	while (result && result.length > 1 && result[3]) {
		...
		match = (result[3][0] === '"') ? result[4] : result[5];
	...
	reGetStringDoubleWithId.lastIndex = 0;
	
	result = reGetStringDoubleWithId.exec(data);
	while (result && result.length > 2 && result[3] && result[4]) {
		// different matches for single and double quotes
		match = (result[3][0] === '"') ? result[4] : result[5];
		key = (result[6][0] === '"') ? result[7] : result[8];

		var last = data.indexOf('\n', reGetStringDoubleWithId.lastIndex);
		last = (last === -1) ? data.length : last;
		var line = data.substring(reGetStringDoubleWithId.lastIndex, last);
		var commentResult = reI18nComment.exec(line);
		comment = (commentResult && commentResult.length > 1) ? commentResult[1] : undefined;
