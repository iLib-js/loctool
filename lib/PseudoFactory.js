...
var PseudoFactory = function(options) {
	...
	if (options.locale === options.project.pseudoLocale && !options.project.settings.nopseudo) {
		logger.trace('Regular pseudo for locale ' + options.locale);
	...
	logger.trace('No pseudo for locale ' + options.locale);
