...
Project.prototype.init = function(cb) {
	...
	this.db.init(function() {
		...
		this.db.getLocales(this.options.id, undefined, function(dbLocales) {
			var locales = this.settings.locales || this.options.locales || dbLocales || [];
			if (!this.settings.locales && !this.settings.nopseudo) {
...
Project.prototype.generatePseudo = function() {
	...
	if (!this.settings.nopseudo) {
		...
		for (var i = 0; i < this.fileTypes.length; i++) {
			if (this.options.generatePseudo) {
				this.fileTypes[i].generatePseudo();
			}
			if (pb) this.fileTypes[i].generatePseudo("en-GB", pb);
