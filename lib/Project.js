    ...
    this.initFileTypes(this.fileTypes, function() {
        ...
        this.db.init(function() {
            ...
            this.db.getLocales(this.options.id, undefined, function(dbLocales) {
                ...
                logger.info("[init] Running time: " + (endTime.getTime() - startTime.getTime()) + " ms .....");
                logger.info("-------------------------------------------------------------------------------------------");
                cb();
            }.bind(this));
        }.bind(this));
...
Project.prototype.extract = function(cb) {
    ...
    this.db.getBy({
        ...
        this.paths = undefined; // signal to the GC that we are done with this
        var endTime = new Date();
        logger.info("[extract] Running time: " + (endTime.getTime() - startTime.getTime()) + " ms .....");
        logger.info("-------------------------------------------------------------------------------------------");
        cb();
    }.bind(this));
};
...
Project.prototype.save = function(cb) {
    ...
    var endTime = new Date();
    logger.info("[save] Running time: " + (endTime.getTime() - startTime.getTime()) + " ms .....");
    logger.info("-------------------------------------------------------------------------------------------");
    cb();
};
...
Project.prototype.generatePseudo = function() {
    ...
    */
    ...
    var endTime = new Date();
    logger.info("[generatePseudo] Running time: " + (endTime.getTime() - startTime.getTime()) + " ms .....");
    logger.info("-------------------------------------------------------------------------------------------");
};
...
Project.prototype.write = function(cb) {
    ...
    this.db.getBy({
        ...
        var endTime = new Date();
        logger.info("[write] Running time: " + (endTime.getTime() - startTime.getTime()) + " ms .....");
        logger.info("-------------------------------------------------------------------------------------------");
        cb();
    }.bind(this));
};
...
Project.prototype.close = function(cb) {
    ...
    this.db.close(function() {
        var endTime = new Date();
        logger.info("[close] Running time: " + (endTime.getTime() - startTime.getTime()) + " ms .....");
        cb();
    });
};
