...
var Project = function(options, root, settings) {
	...
	this.target = this.settings.targetDir || "."; // where localized stuff is written
	this.xliffsDir = this.settings.xliffsDir || "."; // where the xliff files are read from
...
Project.prototype.close = function(cb) {
	...
	for (var i = 0; i < this.fileTypes.length; i++) {
    	...
    	if (this.fileTypes[i].modern && this.fileTypes[i].modern.size() > 0) {
    		logger.info("Writing out the modern translation strings to " + extractedPath);
		...
		for (var i = 0; i < newLocales.length; i++) {
			...
			if (locale !== this.sourceLocale) {
				...
				var resources = newres.getAll().filter(function(res) {
					return (res.origin === "target" && res.locale === locale) && !res.dnt;
				});
				
				if (resources && resources.length) {
					// now find the source for each of these target resources
					var sources = [];
					
					resources.forEach(function(res) {
						sources.push(newres.get(res.hashKeyForTranslation(this.sourceLocale)));
					}.bind(this));
