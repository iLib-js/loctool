...
JavaFile.prototype.extract = function() {
	console.log("JavaFile.extract: Extracting strings from " + this.pathName);
	...
	if (data) {
		...
		while (result && result.length) {
			console.log("JavaFile.extract: Found string " + result[1]);
		...
		while (result && result.length) {
			console.log("JavaFile.extract: Found string " + result[1] + " with unique id " + result[2]);
			var r = new ResourceString({
				id: result[2],
				source: result[1],
				pathName: this.pathName
