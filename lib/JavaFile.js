...
var reGetString = new RegExp('(^R|\\WR)B\\.getString\\s*\\(\\s*"([^"]*)"\\s*\\)[^/\n]*(//\\s*i18n:\\s*(.*))?', "g");
var reGetStringWithId = new RegExp('(^R|\\WR)B\\.getString\\("([^"]*)"\\s*,\\s*"([^"]*)"\\)[^/\n]*(//\\s*i18n:\\s*(.*))?', "g");
...
JavaFile.prototype.parse = function(data) {
	...
	while (result && result.length > 1 && result[2]) {
		...
		if (result[2] && result[2].length) {
			var r = new AndroidResourceString({
