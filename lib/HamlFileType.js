...
HamlFileType.prototype.processHamls = function(translations, locales) {
	...
	*/
	...
	filteredLocales.forEach(function(locale) {
		...
		/*
		var tryitnow = translations.get(ResourceString.hashKey(this.project.getProjectId(), locale, "r200169172"), "x-haml");
	...
	for (var i = 0; i < this.files.length; i += 100) {
		...
		if (procStatus.status === 0) {
			...
			if (fs.existsSync("unmapped.yml")) {
				...
				filteredLocales.forEach(function(locale) {
					logger.info("Adding target unmappeds in locale " + locale);
					...
					logger.info("Adding target unmappeds in locale " + JSON.stringify(targets, undefined, 4));
