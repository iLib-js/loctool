 ...
 */
var LocalRepository = function (sourceLocale, pathName) {
 ...
 * @param {Function} cb the call back to call when the initialization is done
 */
LocalRepository.prototype.init = function(projectName, cb) {
	var fileName = path.join(this.path, projectName + ".xliff");
	var xliff = new Xliff({
		sourceLocale: this.sourceLocale
	});
	var data = fs.readFileSync(fileName, "utf-8");
	this.set = xliff.deserialize(data);
	cb();
};
...
LocalRepository.prototype.close = function(cb) {
	var fileName = path.join(this.path, "all.xliff");
	var xliff = new Xliff({
		sourceLocale: this.sourceLocale
	});
	xliff.addSet(this.ts);
	fs.writeFileSync(fileName, xliff.serialize(), "utf-8");
	cb();
};
