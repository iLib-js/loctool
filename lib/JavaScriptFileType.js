...
JavaScriptFileType.prototype.write = function(translations, locales) {
	...
	for (var i = 0; i < resources.length; i++) {
		res = resources[i];
		if (res.locale !== this.project.sourceLocale) {
		...
		translationLocales.forEach(function(locale) {
			...
			db.getResourceByHashKey(res.cleanHashKeyForTranslation(locale), function(err, translated) {
				var r = translated;
				if (!translated) {
					r = res.clone();
					r.locale = locale;
...
JavaScriptFileType.prototype.registerDataTypes = function() {
	ResourceFactory.registerDataType("javascript", "string", ResourceString);
};
