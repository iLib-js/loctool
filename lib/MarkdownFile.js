...
MarkdownFile.prototype._emitText = function(escape) {
    ...
    logger.trace('text: pre is "' + pre + '" value is "' + text + '" and post is "' + post + '"');
...
//                            var tag = {
//                                name: tagName,
//                                index: this.message.getCurrentLevel()
//                            };
//
 ...
 * @private
 */
MarkdownFile.prototype._walkLocalize = function(node, locale, translations) {
    var match, valid;
        ...
        case 'strong':
            ...
            if (node.children && node.children.length) {
                ...
                node.children.forEach(function(child, index, array) {
                    ...
                    if (valid) {
                        this._walkLocalize(child, locale, translations);
                    }
                }.bind(this));
            }
            if (doPushPop) {
...
MarkdownFile.prototype.localizeText = function(translations, locale) {
    ...
    // make a complete copy by using filter but then not filtering anything out
    var newAst = unistFilter(this.ast, function(node) {
        return true;
    });

    this._walkLocalize(newAst, locale, translations);

    // in case any is left over at the end
    this._localizeText(locale);
