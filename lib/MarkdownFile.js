...
Markdown.prototype._parseHtml = function(text) {
    ...
    htmlParser.parse(data, {
        ...
        attribute: function(name, value, quote) {
            ...
            if ((name === "title" || (utils.localizableAttributes[lastTagName] && utils.localizableAttributes[lastTagName][name])) &&
                ...
                if (tagtext) {
                    tagtext += " " + name + '=' + quote + utils.escapeQuotes(value) + quote;
                } else if (text) {
                    text += " " + name + '=' + quote + utils.escapeQuotes(value) + quote;
                }
            }
        }.bind(this),
        text: function(value) {
            logger.trace('text: value is "' + value + '"');
            if (utils.isAllWhite(value)) {
                if (tagtext) {
                    tagtext += value;
                } else if (text) {
                    text += value;
...
MarkdownFile.prototype.parse = function(data) {
    ...
    var parser = unified().use(markdown, {commonmark: true}).use(html);
