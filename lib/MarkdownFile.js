...
/**
 * Parse readme.io file headers and create new trans units out of it.
 *
 * @param {string} header the header to parse
 * @private
 */
MarkdownFile.prototype._processHeader = function(header) {
    headerRe.lastIndex = 0;
    var match = headerRe.exec(header);
    while (match) {
        var text = match[2];
        if (isTranslatable(text)) {
            this._addTransUnit(text, 'This is a header with label: ' + match[1]);
        }

        match = headerRe.exec(header);
    }
};
 ...
 * Parse readme.io file headers and create new trans units out of it.
 *
 * @param {string} blockText the json block to parse
 * @private
 */
MarkdownFile.prototype._processBlock = function(blockText) {
    /*
    var block = JSON.parse(blockText);
    if (isTranslatable(text)) {
        this._addTransUnit(text, 'This is a header with label: ' + match[1]);
    }
    */
...
MarkdownFile.prototype._walk = function(node) {
    var match, valid, blockText, headerText;
    ...
    switch (node.type) {
        ...
        default:
            if (this.state === "md") this._emitText();
            if (node.children && node.children.length) {
                valid = true;
                    ...
                    if (child.type === 'linkReference') {
                        ...
                        if (index+1 < array.length && array[index+1]) {
                            ...
                            if (p.type === 'paragraph' &&
                                ...
                                valid = true;
                                this.state = "md";
                                this._processHeader(headerText);
                            }
                        } else {
                            valid = true;
                            this.state = "md";
 ...
 * @private
 */
MarkdownFile.prototype._localizeHeader = function(headerText) {
 ...
 * @private
 */
MarkdownFile.prototype._localizeBlock = function(blockText) {
...
MarkdownFile.prototype.localizeText = function(translations, locale) {
    ...
    var state = "md", valid = true, start = -1, end, ma = new MessageAccumulator();
    var blockText, headerText;
    ...
    for (var i = 0; i < nodeArray.length; i++) {
        if (nodeArray[i].type === 'linkReference') {
            ...
            if (i+2 < nodeArray.length && nodeArray[i+1] && nodeArray[i+2]) {
                if (nodeArray[i+1].type === 'paragraph' &&
                    ...
                    valid = true;
                    state = "md";
                    this._localizeHeader(headerText);
                }
            } else {
                valid = true;
                state = "md";
        ...
        switch (state) {
            ...
            case 'block':
                blockText += nodeArray[i].value;
                break;

            case 'header':
                headerText += nodeArray[i].value;
                break;
        }
