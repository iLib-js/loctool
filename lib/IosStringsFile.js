...
IosStringsFile.prototype.getContent = function() {
	...
	for (var j = 0; j < resources.length; j++) {
		...
		content += '"' + escapeString(thiskey) + '" = "' + escapeString(resource.getTarget()) + '";\n';
	}
...
/**
 * Return the directory to use with this locale
 */
IosStringsFile.prototype.makePathName = function() {
	if (!this.pathName) {
		...
		var locale = this.locale || this.project.sourceLocale;
		var mapped = localeMapping[locale] || (new Locale(locale).language + ".lproj");
		var resDir = (this.project.options && this.project.options.resourceDirs && this.project.options.resourceDirs["objc"]) || ".";
		var dir = path.join(this.project.root, resDir, mapped);
