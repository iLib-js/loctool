...
IosStringsFile.prototype.addResource = function(res) {
	...
	if (res && 
		...
		// in order to merge strings from all files together, we have to add it to the translation set without the file name or any
		// of the other properties that differentiate the strings.
		this.set.add(new ResourceString({
			project: res.project,
			reskey: res.reskey,
			text: res.text,
	        comment: res.comment,
			datatype: "x-ios-strings"
		}));
	} else {
		if (res) {
