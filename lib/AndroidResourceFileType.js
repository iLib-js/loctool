...
var AndroidResourceFileType = function(project) {
	...
	if (this.project && this.project.settings && this.project.settings["build.gradle"]) {
		var bg = new BuildGradle({
			project: this.project,
			path: this.project.settings["build.gradle"]
		});
...
AndroidResourceFileType.prototype.write = function(translations, locales) {
	...
	for (var i = 0; i < resources.length; i++) {
		res = resources[i];
		file = this.getResourceFile(res.context, res.locale, res.resType + "s", res.original);
		file.addResource(res);
		logger.trace("Added " + res.reskey + " to " + file.pathName);
	}
...
AndroidResourceFileType.prototype.getResourceFile = function(context, locale, type, original) {
	...
	if (this.flavors) {
		...
		var stripped = original.substring(path.join(this.project.getRoot()).length+1);
		for (var f in this.flavors) {
			if (stripped.startsWith(this.flavors[f].src) || stripped.startsWith(this.flavors[f].res)) {
				flavor = f;
				break;
			}
		}
	}
	var key = [(context || "default"), (locale || "default"), type, flavor].join("_");
	...
	if (!resfile) {
		...
		var resdir = (flavor !== "main") ? this.flavors[flavor].res[0] : this.project.options.resourceDirs["java"];
