...
AndroidResourceFileType.prototype.generatePseudo = function() {
	...
	resources.forEach(function(resource) {
		if (resource.getId() !== "app_id" && resource.getId() !== "live_sdk_client_id") {
			var res = resource.generatePseudo(locale.getSpec(), this.pseudoBundle);
			this.translationSet.addResources(res);
		} else {
			var res = resource.clone({
				locale: locale.getSpec()
			});
			
			this.translationSet.addResource(res);
		}
	}.bind(this));
};
...
AndroidResourceFileType.prototype.newFile = function(pathName) {
	...
	if (!this.resourceFiles[context]) {
		this.resourceFiles[context] = {};
	}
	
	this.resourceFiles[context][locale] = file;
	return file;
};
 ...
 */
AndroidResourceFileType.prototype.getResourceFile = function(context, locale) {
	var resfile = this.resourcefiles && this.resourcefiles[context] &&
			this.resourcefiles[context][locale];
	
	if (!resfile) {
		if (!this.resourcefiles[context]) {
			this.resourcefiles[context] = {};
		}
		resfile = this.resourcefiles[context][locale] = new AndroidResourceFile({
			project: this.project,
			context: context,
			locale: locale
		});
