...
var PseudoHant = function(options) {
	options.sourceLocale = "zh-Hans-CN";
	this.parent(options);
	
	// Taiwan uses a slightly different style than Hong Kong, so switch dictionaries appropriately
	this.occ = new OpenCC(this.locale === "zh-Hant-TW" ? "s2twp.json" : "s2hk.json");
};
...
PseudoHant.prototype.getStringForResource = function(resource, selector) {
	...
	var source = (resource.resType === "plural" || resource.resType === "array") ? simplified.getSource(selector) : simplified.getSource();
	return simplified && source ? this.occ.convertSync(source) : source;
};
