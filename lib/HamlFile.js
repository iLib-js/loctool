...
var entjson = fs.readFileSync("../db/entities.json", "utf-8");
...
var HamlFile = function(options) {
	...
	this.modern = new TranslationSet(this.project ? this.project.sourceLocale : "zxx-XX");
};
		...
		if ((line[lineIndent] === '!' || line[lineIndent] === '&') && lineIndent < line.length && line[lineIndent+1] === '=') {
			...
			case '#':
				...
				if (line[lineIndent] === "%" && this.localizedBuffer.length > 0) {
					...
					if (utils.nonBreakingTags[tag] && lineIndent >= currentIndent) {
...
HamlFile.prototype.assembleTranslation = function(segment, translations, locale) {
	...
	if (translated) {
		// modern translation
		return translated.getSource();
	}
	
	// need to cobble together old translations
	var text = HamlFile.unescapeString(segment.original);
	var match, lines = [], all = true;
	...
	var translations = lines.map(function(line) {
		if (line[0] === "<") {
			// html, just return it unaltered
			return line;
		}
		var translated = translations.getClean(
			ResourceString.cleanHashKey(
		...
		all = all || !!translated;
		return translated && translated.getSource() || line;
	}.bind(this));
	...
	if (all) {
		...
		for (var i = 1; i < translations.length; i++) {
			if ((translations[i-1][0] !== '<' || translations[i-1][1] === '/') && translations[i].substring(0, 2) !== '</') {
				translated += joinchar;
			}
	...
	if (translated) {
		...
		this.modern.add(new ResourceString({
			project: this.project.getProjectId(),
			locale: this.project.sourceLocale,
