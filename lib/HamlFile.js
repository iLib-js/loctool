...
HamlFile.prototype.assembleTranslation = function(segment, translations, locale) {
	...
	var translationList = lines2.map(function(line) {
		...
		all = all && !!translated;
		return translated && translated.getSource();
	}.bind(this));
	...
	translated = translationList[0];
	for (var i = 1; i < translationList.length; i++) {
		if ((translationList[i-1][0] !== '<' || translationList[i-1][1] === '/' || translationList[i].substr(-2) === "/>") && 
				translationList[i].substring(0, 2) !== '</') {
			translated += joinchar;
		}
		translated += translationList[i];
	}
