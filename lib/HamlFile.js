 ...
 * @returns {Number} the index of the first character
 * on the line that may contain localizable text
 */
HamlFile.prototype.firstLocalizable = function(line, start) {
	var first = start;
	
	while (first < line.length-1 && line[first] !== ' ' && line[first] !== '\t' && !(line[first] in brackets)) {
		first++;
	}
	
	if (line[first] in brackets) {
		first = this.findMatching(line, first);
	}
...
HamlFile.prototype.parse = function(data) {
	...
	while (i < lines.length) {
		...
		case '%':
			...
			this.emitNonLocalized(line.substring(0, end));
