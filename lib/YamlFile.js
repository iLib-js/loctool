...
YamlFile.prototype._mergeOutput = function(prefix, obj, set){
	for (var key in obj) {
		if (typeof(obj[key]) === "object") {
			...
			var resource = obj[key];
			if (resource && typeof(resource) === "string" && (resource.indexOf(' ') > -1 || resource.indexOf('_') === -1)) {
				logger.trace("Found string resource " + JSON.stringify(resource) + " locale " + this.getLocale());
				existing = set.get(key);
				if (existing && !existing.getLocalize()){
					logger.trace("Keeping source for " + key + " locale " + this.getLocale());
		...
		if (typeof(obj[key]) === "object") {
			...
			if (resource && typeof(resource) === "string" && (resource.indexOf(' ') > -1 || resource.indexOf('_') === -1)) {
				...
				if (locale === this.project.pseudoLocale) {
					ret[key] = this.type.pseudoBundle.getStringJS(obj[key].toString());
				} else {
