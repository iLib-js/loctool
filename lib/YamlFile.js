	...
	for (var key in obj) {
		if (typeof(obj[key]) === "object") {
			...
			if (resource && typeof(resource) === "string" && (resource.indexOf(' ') > -1 || resource.indexOf('_') === -1)) {
				...
				var resources = set.getBy({reskey: key, pathName: this.pathName, targetLocale: this.getLocale(), project: this.project.getProjectId()});
		...
		for (var j = 0; j < resources.length; j++) {
			...
			if (resource.resType === "plural" || resource.getSource()) {
				...
				if (resource.resType === "plural") {
					logger.trace("writing plural translation for " + resource.getKey() + " as " + JSON.stringify(resource.getTargetPlurals()));
					parent[key] = resource.getTargetPlurals();
				} else {
					logger.trace("writing translation for " + resource.getKey() + " as " + resource.getTarget());
					parent[key] = resource.getTarget();
				}
			} else {
			...
			if (resource && typeof(resource) === "string" && (resource.indexOf(' ') > -1 || resource.indexOf('_') === -1)) {
				...
				if (locale === this.project.pseudoLocale && this.project.settings.nopseudo) {
					...
					if (sourceLocale !== this.project.sourceLocale) {
						...
						source = sourceRes ? sourceRes.getSource() : obj[key].toString();
					} else {
