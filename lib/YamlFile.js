...
YamlFile.prototype._parseResources = function(prefix, obj, set) {
	for (var key in obj) {
		if (typeof(obj[key]) === "object") {
			...
			this._parseResources(pre + key, obj[key], set);
		} else {
...
YamlFile.prototype.parse = function(str) {
	this._parseResources("", YAML.load(str), this.set);
};
...
YamlFile.prototype.getContent = function() {
	...
	if (this.set.isDirty()) {
		...
		for (var j = 0; j < resources.length; j++) {
			...
			if (resource.getSource()) {
				...
				var mungedLocale = resource.getLocale().replace(/-/g, "_");
				if (!json[mungedLocale]) {
					json[mungedLocale] = {};
				}
				json[mungedLocale][resource.getKey()] = resource.getSource();
			} else {
	...
	return yamljs.stringify(json, 2);
};
