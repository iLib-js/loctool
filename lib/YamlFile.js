...
function readResFile(pathName) {
	...
	return YAML.parse(yml, {object: true});
}
	...
	if (this.pathName) {
		try {
			...
			*/
			...
			logger.trace("Resources are: " + JSON.stringify(this.set.getAll(), undefined, 4));
			logger.trace("YamlFile: loaded strings in " + this.pathName);
		} catch (e) {
			logger.warn("Could not read " + this.pathName + "\n" + e);
		}
	}
...
var lang = /\/([a-z][a-z]).yml$/;
var reg = /\/([a-z][a-z]-[A-Z][A-Z]).yml$/;
var script = /\/([a-z][a-z]-[A-Z][a-z][a-z][a-z]-[A-Z][A-Z]).yml$/;

/**
 * Parse the suffixes of the resource file for the locale name. The 
 * locale for yaml resource files
 * is given in the file name of that the resource file.<p>
 * 
 * The general syntax of the file name is:<p>
 * 
 * <pre>
 * language [ "-" region | "-" script "-" region]]
 * </pre>
 * 
 * @private
 */
