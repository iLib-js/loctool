...
ResourceString.prototype.generatePseudo = function(locale, pseudoBundle) {
	...
	var r = this.clone();
	r.locale = locale;
	r.text = utils.isAndroidResource(this.text) ? this.text : pseudoBundle.getStringForResource(this);
	r.origin = "target";
 ...
 * @return {String} a hash key
 */
ResourceString.cleanHashKey = function(project, locale, reskey, datatype) {
	var cleaned = reskey && reskey.replace(/\s+/g, " ").trim() || "";
	var key = ["rs", project, locale, cleaned, datatype].join("_");
...
ResourceString.prototype.cleanHashKey = function() {
	return ResourceString.cleanHashKey(this.project, this.locale, this.reskey, this.datatype);
};
...
ResourceString.prototype.cleanHashKeyForTranslation = function(locale) {
	return ResourceString.cleanHashKey(this.project, locale, this.reskey, this.datatype);
};
