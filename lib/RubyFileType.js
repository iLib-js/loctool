...
var RubyFileType = function(project) {
	...
	// generate all the automatic bundles we'll need
...
RubyFileType.prototype.checkAllPluralCases = function (sourceRes, res, locale) {
	...
	if (res.resType === "plural") {
		...
		var items = sourceRes.getPlurals();
		var newItems = {};
		
		var translatedItems = res.getPlurals();
		for (var p in items) {
			var item = items[p];
		...
		if (!fullyTranslated) {
			...
			var newSourceRes = sourceRes.clone();
			newSourceRes.strings = newItems;
			newSourceRes.origin = "source";
			this.newres.add(newSourceRes);
