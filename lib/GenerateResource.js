...
GenerateResource.prototype._calResourcePath = function(resLocale) {
    var resPath = "";
...
GenerateResource.prototype.getResourceFilePath = function(resLocale) {
    var resPath = this._calResourcePath(resLocale);
    return path.join(this.targetDir, this.resRoot, resPath, this.resName);
}

GenerateResource.prototype._writeFiles = function() {
    var fullPath;
    for (locale in this.output){
        var resourcePath = this._calResourcePath(locale);
        var content = JSON.stringify(this.output[locale], true, 4);

        utils.makeDirs(path.join(this.targetDir, this.resRoot, resourcePath));
        var fullPath = this.getResourceFilePath(locale, resourcePath);
...
GenerateResource.prototype.generate = function() {
    if (!this.ts.resources.length) return;

    var resources = this.ts.resources;
    logger.info("Generating resource size: " + this.ts.resources.length);
