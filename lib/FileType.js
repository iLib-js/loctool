 ...
 * 
 * @param {String} sourceLocale the source locale from which the target resources are generated
 * @param {String} targetLocale the locale that the target locales will have
 * @param {ResBundle} pb the pseudo translation bundle used to derived the resources
 */
FileType.prototype.generatePseudo = function(locale, pb) {
	logger.trace("generate pseudo and derived resources for file type " + this.name() + " source locale " + sourceLocale + " target locale " + targetLocale);
	
	var resources = this.extracted.getBy({
		locale: pb.getSourceLocale()
	});
