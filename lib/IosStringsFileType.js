	...
	for (var i = 0; i < resources.length; i++) {
		...
		locales.forEach(function(locale) {
			db.getResource(res.getKey(), res.getType(), res.getContext(), locale, res.getProject(), res.getPath(), function(err, translated) {
				var r = (!translated || res.dnt) ? res : translated; // default to the source language if the translation is not there
				if (!translated) {
					logger.trace("No translation for " + res.reskey + " to " + locale);
				}
				
				file = this.getResourceFile(locale, r.pathName, "xib");
				file.addResource(r);
...
IosStringsFileType.prototype.getResourceFile = function(locale, pathName, type) {
	...
	if (locale === this.project.pseudoLocale || locale === this.project.sourceLocale) {
		localeDir = locale;
	} else {
