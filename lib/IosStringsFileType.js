...
IosStringsFileType.prototype.write = function(translations, locales) {
	...
	for (var i = 0; i < resources.length; i++) {
		res = resources[i];
		file = this.getResourceFile(res.getTargetLocale(), res.pathName, "xib");
 ...
 */
IosStringsFileType.prototype.getResourceFilePath = function(locale, pathName, type) {
	var l = new Locale(locale || this.project.sourceLocale);
	var localeDir, dir, newPath;
	var root = this.project.getRoot() || ".";
	...
	localeDir = localeMapping[locale] || l.language + ".lproj";
	...
	if (type === "xib") {
		...
		newPath = path.join(root, dir, localeDir, base + ".strings");
	} else {
		dir = this.project.options.resourceDirs["objc"] || ".";
		newPath = path.join(root, dir, localeDir, "Localizable.strings");
	}
 ...
 * @param {String} type one of "objc" or "xib" strings from each source
 * file type go into different types of resource files
 * @return {IosStringsFile} the iOS resource file that serves the
 * given project, context, and locale.
 */
IosStringsFileType.prototype.getResourceFile = function(locale, pathName, type) {
	var root = this.project.getRoot() || ".";
	var newPath = this.getResourceFilePath(locale, pathName, type);

	utils.makeDirs(path.dirname(newPath));
