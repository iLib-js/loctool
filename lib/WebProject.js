...
var WebProject = function(options, root, settings) {
	...
	this.defaultLocales = settings.locales || options.settings.locales;
...
WebProject.prototype.extract = function(cb) {
	this.parent.prototype.extract.call(this, function() {
		this.db.getBy({
			...
			var translations = new TranslationSet(this.sourceLocale);
			translations.addAll(resources);
			
			logger.trace("Extracting haml strings from project " + this.options.id);
			this.hamlType.processHamls(translations, this.locales);
			cb();
		}.bind(this));
	}.bind(this));
};
