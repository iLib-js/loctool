...
YamlResourceFile.prototype.getResourceFilePath = function(locale, flavor) {
	...
	var langscript = l.language + "-" + l.script, 
		def = localeDefaults[langscript];
	
	if (def !== locale) {
		langscript = l.language;
		def = localeDefaults[l.language];
		if (def !== locale) {
...
YamlResourceFile.prototype.write = function() {
	...
	if (this.set.isDirty()) {
		...
		if (!this.pathName) {
			...
			dir = path.join(this.project.target, this.project.options.resourceDirs["yml"]);
