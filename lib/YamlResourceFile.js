...
YamlResourceFile.prototype._handleResourceString = function(resource, object) {
    var string = resource.getTarget() || resource.getSource();
    if (string) {
        logger.trace("writing string " + resource.getKey() + " as " + resource.getSource());
        // var locale = resource.getLocale().replace(/-/g, "_");
        var locale = resource.getTargetLocale() || resource.getSourceLocale();
...
YamlResourceFile.prototype._handleResourcePlural = function(resource, object) {
    ...
    var plurals = resource.getTargetPlurals() || resource.getSourcePlurals();
    if (resource.getClasses() && plurals.one && plurals.other) {
        logger.trace("writing plurals " + resource.getKey() + " with " + JSON.stringify(resource.getClasses()));
        var locale = resource.getTargetLocale() || resource.getSourceLocale();
        var key = resource.getKey();
        object[key] = plurals;
    } else {
    ...
    for (var key in parsed) {
        ...
        if (localeSpec.exec(spec)) {
            ...
            if (Locale.a1toa3langmap[l.getLanguage()] && (!l.getRegion() || Locale.a2toa3regmap[l.getRegion()])) {
                this.locale = new Locale(l.language, l.region, undefined, l.script).getSpec();
...
YamlResourceFile.prototype.getPath = function() {
    return this.pathName || this.getResourceFilePath();
};
 ...
 * @private
 */
YamlResourceFile.prototype.getLocaleDefault = function(locale, flavor) {
    var l = new Locale(locale || this.locale);
    flavor = flavor || this.flavor || l.variant;
...
YamlResourceFile.prototype.getContent = function() {
    ...
    if (this.set.isDirty()) {
        ...
        var defaultLoc = this.getLocaleDefault();
        if (resources.length) {
            json[defaultLoc] = {};
        }
...
YamlResourceFile.prototype.getResourceFilePath = function(locale, flavor) {
    var localeDir, dir, newPath, spec = this.getLocaleDefault();
