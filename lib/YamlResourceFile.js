...
YamlResourceFile.prototype._handleResourceString = function(resource, object){
	if (resource.getSource()) {
		...
		var locale = resource.getLocale();
		if (!object[locale]) {
			object[locale] = {};
		}
		var key = resource.getKey();
		object[locale][key] = resource.getSource();
	} else {
...
YamlResourceFile.prototype._handleResourcePlural = function(resource, object){
	...
	if (resource.getClasses() && resource.get("one") && resource.get("other")) {
		...
		var key = resource.getKey();
		if (!object[locale][key]){
			object[locale][key] = {};
		}
		var strings = resource.getClasses();
		for (var i = 0; i < strings.length; i++){
			var pluralCase = strings[i];
			var pluralString = resource.get(strings[i]);
			object[locale][key][pluralCase] = pluralString;
		}
	} else {
