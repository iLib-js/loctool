	...
	if (this.set.isDirty()) {
		...
		for (var j = 0; j < resources.length; j++) {
			...
			if (resource instanceof ContextResourceString || resource instanceof ResourceString) {
				...
				var items = resource.getTargetArray();
				...
				for (var quantity in plurals.getTargetPlurals()) {
					arr.item.push({
						quantity: quantity,
						"$t": escapeString(plurals.getTarget(quantity))
					});
				}
...
AndroidResourceFile.prototype.write = function() {
	...
	if (this.set.isDirty()) {
		...
		if (alternateDirs && alternateDirs[this.locale]) {
			var baseDir = path.dirname(path.dirname(this.pathName));
			...
			for (var altDir in dirs) {
				...
				var pathName = path.join(alternateDir, fileResType + "s.xml");
