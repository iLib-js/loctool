...
function readResFile(pathName) {
	logger.trace("Attempting to read and parse file " + pathName);
	...
	if (this.pathName) {
		...
		if (resources.string) {
			...
			for (var i = 0; i < strArr.length; i++) {
				...
				if (strArr[i].i18n && utils.isDNT(strArr[i].i18n)) {
					logger.trace("This string marked as 'do not translate'.")
				} else if (utils.isAndroidResource(strArr[i])) {
		...
		if (resources["string-array"]) {
			...
			for (var i = 0; i < arrays.length; i++) {
				strArr = arrays[i];
				if (strArr.i18n && utils.isDNT(strArr.i18n)) {
					logger.trace("This array marked as 'do not translate'.")
				} else if (!strArr.item || strArr.item.every(function(item) { return utils.isAndroidResource(item);})) {
					// note that this is a used resource?
					logger.trace("Already resourcified");
				} else {
		...
		if (resources.plurals) {
			...
			for (var i = 0; i < plurals.length; i++) {
				...
				var items = {};
				if (strArr && strArr.i18n && utils.isDNT(strArr.i18n)) {
					logger.trace("Plurals marked as 'do not translate'.");
				} else if (strArr && strArr.item && ilib.isArray(strArr.item)) {
