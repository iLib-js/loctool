	...
	if (!this._pathParsed)  {
		...
		if (!this.locale) {
			...
			if ((match = script.exec(dir)) && match && match.length > 0) {
				...
				this.locale = new Locale(match[1], match[3], match[2]);
				this.context = match.length > 5 ? match[5] : undefined;
			} else if ((match = reg.exec(dir)) && match && match.length > 0) {
				//this.locale = new Locale(dir.substring(dir.length-6).replace("-r", "-")).getSpec();
				this.locale = new Locale(match[1] + "-" + match[2]);
				this.context = match.length > 4 ? match[4] : undefined;
			} else if ((match = lang.exec(dir)) && match && match.length > 0) {
				// this.locale = new Locale(dir.substring(dir.length-2)).getSpec();
				this.locale = new Locale(match[1]);
				this.context = match.length > 3 ? match[3] : undefined;
			}
