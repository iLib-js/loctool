...
// the locales that go into the values language dirs. If we have a version of Spanish, for example es-ES, that is not 
	...
	if (this.set.isDirty()) {
		...
		if (!this.pathName) {
			...
			if (this.locale !== this.project.sourceLocale) {
				...
				// for Android 7+ to use with the fully specified locale specifier
				if (alternateDirs[this.locale]) {
					locale = new Locale(alternateDirs[this.locale]);
					
					alternateDir = valueDir + "-b+" + locale.getLanguage() + "+" +  locale.getScript() + "+" + locale.getRegion();
				}
				
				locale = new Locale(this.locale);
				valueDir += "-" + locale.getLanguage();
				if (this.locale === this.project.pseudoLocale || this.locale !== defaultLocales[locale.getLanguage()]) {
			...
			if (alternateDir) {
				...
				adir = path.join(this.project.root, this.project.options.resourceDirs["java"], alternateDir);
			}
		...
		logger.trace("Set is: " + JSON.stringify(resources, undefined, 4));
		...
		if (adir) {
			var pathName = path.join(adir, fileResType + "s.xml");
