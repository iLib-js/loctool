	...
	if (this.set.isDirty()) {
		...
		if (!this.pathName) {
			...
			if (this.locale !== this.project.sourceLocale) {
				...
				if (this.locale !== defaultLocales[locale.getLanguage()]) {
					if (locale.getScript()) {
						valueDir += "-" + locale.getLanguage() + "-r" + locale.getRegion() + "_#" + locale.getScript();
					} else {
						if (locale.getRegion()) {
					...
					// for the default locale for a language, use the language code only
					valueDir += "-" + locale.getLanguage();
				}
		...
		fs.writeFileSync(this.pathName, this._getXML(), "utf8");
