...
var reWrongReplacementType = new RegExp(/[^#|\W#]\{/);

var reGetStringSingle = /(^R|\WR)b\.t\s*\(\s*'((\\'|[^'])*)'\s*(\)|,)/g;
var reGetStringDouble = /(^R|\WR)b\.t\s*\(\s*"((\\"|[^"])*)"\s*(\)|,)/g;
...
RubyFile.prototype.parse = function(data) {
	...
	while (result && result.length > 1 && result[2]) {
		...
		if (result[2]) {
			var last = data.indexOf('\n', reGetStringDouble.lastIndex);
			last = (last === -1) ? data.length : last;
			var line = data.substring(reGetStringDouble.lastIndex, last);
			...
			while (tmp) {
				logger.warn("Warning: Ruby #{} parameters are not allowed in the RB.t() parameters:");
				logger.warn(tmp[0]);
			}
	...
	while (result && result.length > 1 && result[2]) {
		logger.trace("Found string key: " + this.makeKey(result[2]) + ", string: '" + result[2] + "', comment: " + (result.length > 5 ? result[5] : undefined));
		if (result[2] && result[2].length > 2) {
			var last = data.indexOf('\n', reGetStringSingle.lastIndex);
			last = (last === -1) ? data.length : last;
			var line = data.substring(reGetStringSingle.lastIndex, last);
			var commentResult = reI18nComment.exec(line);
			comment = (commentResult && commentResult.length > 1) ? commentResult[1] : undefined;
	...
	while (result && result.length > 1 && result[2]) {
		...
		if (preResource['one']){
			...
			for (var quantity in preResource) {
				if (ResourcePlural.validPluralClasses.includes(quantity)) {
					...
					r.addString(quantity, RubyFile.unescapeString(preResource[quantity]));
				} else{
					logger.warn("Warning: ruby plural resource with invalid quantity:" + quantity);
	...
	while (result) {
		logger.warn("Warning: string concatenation is not allowed in the RB.t() parameters:");
		logger.warn(result[0]);
		result = reGetStringBogusConcatenation1.exec(data);
	}
	...
	while (result) {
		logger.warn("Warning: non-string arguments are not allowed in the RB.t() parameters:");
		logger.warn(result[0]);
		result = reGetStringBogusParam.exec(data);
	}
};
