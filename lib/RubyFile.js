 ...
 * file
 */
var RubyFile = function(project, pathName) {
...
RubyFile.prototype.makeKey = function(source) {
	...
	var value = source.replace(/\s\s*/, '_');
	value = 
	logger.trace("String '" + source + "' hashes to " + value);
...
var reGetString = new RegExp('(^R|\\WR)b\\.t\\s*\\(\\s*"([^"]*)"\\s*\\)[^/\n]*(#\\s*i18n:?\\s*(.*))?', "g");
var reGetStringWithId = new RegExp('(^R|\\WR)b\\.t\\("([^"]*)"\\s*,\\s*"([^"]*)"\\)[^/\n]*(#\\s*i18n:?\\s*(.*))?', "g");
