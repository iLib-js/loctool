...
var reGetStringPluralArrowDouble = /(^R|\WR)b\.p\s*\(\s*(:\w+\s*=>\s*"((\\"|[^"])*)"\s*(\)|,)\s*)\s*(:\w+\s*=>\s*"((\\"|[^"])*)"\s*(\)|,)\s*)/g;
...
RubyFile.prototype.parse = function(data) {
	...
	reGetStringPluralArrowDouble.lastIndex = 0;
	var result = reGetStringPluralArrowDouble.exec(data);
	while (result && result.length > 1 && result[2]) {
		logger.trace("Found string key: " + this.makeKey(result[2]) + ", string: '" + result[2] + "', comment: " + (result.length > 5 ? result[5] : undefined));
		...
		result = reGetStringPluralArrowDouble.exec(data); // for safety
	}
