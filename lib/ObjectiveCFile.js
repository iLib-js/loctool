...
var reNSLocalizedStringBogusConcatenation1 = new RegExp(/\bNSLocalizedString\s*\(\s*@"[^"]*"\s*\+/g);
var reNSLocalizedStringBogusConcatenation2 = new RegExp(/\bNSLocalizedString\s*\([^\)]*\+\s*@"[^"]*"\s*\)/g);
var reNSLocalizedStringBogusParam = new RegExp(/\bNSLocalizedString\s*\([^"\)]*\)/g);

var reNSLocalizedString = new RegExp('\\bNSLocalizedString\\s*\\(\\s*@"([^"]*)"\\s*,\\s*(nil|@"([^"]*)")\\s*\\)', "g");
var reNSLocalizedStringWithParams = new RegExp('\\bNSLocalizedString\\("([^"]*)",\\s*"([^"]*)"\\)[^/\n]*(//\\s*i18n:\\s*(.*))?', "g");
...
ObjectiveCFile.prototype.parse = function(data) {
	...
	while (result && result.length > 1 && result[1]) {
		logger.trace("Found string key: " + this.makeKey(result[1]) + ", string: '" + result[1] + "', comment: " + (result.length > 3 ? result[3] : undefined));
