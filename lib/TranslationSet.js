 ...
 * @returns
 */
TranslationSet.prototype._hashKey = function(project, context, locale, key) {
	var key = [key, locale || this.sourceLocale, context, project].join("_");
...
TranslationSet.prototype.get = function(key, context, locale) {
	...
	return this.byKey[this._hashKey(undefined, context, locale, key)];
};
...
TranslationSet.prototype.add = function(resource) {
	...
	hashKey = this._hashKey(resource.project, resource.context, resource.locale, key);
	
	existing = this.byKey[hashKey];
	
	if (existing) {
		logger.trace("Same key as existing resource");
		...
		logger.trace("Done adding resource");
	}
};
...
TranslationSet.prototype.addSet = function(set) {
	if (set && set.resources & set.resources.length) {
		...
		logger.trace("addSet: resources is now: " + JSON.stringify(this.resources));
	}
};
