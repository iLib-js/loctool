 ...
 * @returns
 */
TranslationSet.prototype._hashKey = function(project, context, locale, key, type, pathName) {
	var key = [key, type || "string", locale || this.sourceLocale, context, project, pathName].join("_");
 ...
 */
TranslationSet.prototype.get = function(key, type, context, locale, project, pathName) {
	logger.trace("Get a resource");
	return this.byKey[this._hashKey(project, context, locale, key, type, pathName)];
};
...
TranslationSet.prototype.add = function(resource) {
	...
	hashKey = this._hashKey(resource.project, resource.context, resource.locale, key, resource.resType, resource.pathName);
	...
	if (resource && resource.project && resource.context && resource.locale && resource.reskey && resource.resType) {
		for (var i = 0; i < this.resources.length; i++) {
			...
			if (resource.project === res.project &&
				...
				var hashKey = this._hashKey(resource.project, resource.context, resource.locale, resource.reskey, resource.resType, resource.pathName);
...
TranslationSet.prototype.diff = function(other) {
	...
	for (var i = 0; i < otherres.length; i++) {
		res = otherres[i];
		existing = this.byKey[this._hashKey(res.project, res.context, res.locale, res.reskey, res.resType, res.pathName)];
