...
TranslationSet.prototype.add = function(resource) {
	...
	if (existing) {
		...
		logger.info("New resource " + hashKey + " " + cleanKey + " " + resource.getSource());
		this.resources.push(resource);
		this.byKey[hashKey] = resource;
		if ( existingClean ){
			logger.trace("Overwrite cleanKey " + cleanKey + " with " + resource.getSource());
		} else {
			logger.trace("New cleanKey " + cleanKey + " with " + resource.getSource());
